        ...
        items.push({
            ...
            listeners: {
                click: function(btn, e, eOpts) {
                    this.ownerCt.ownerCt.doDoneAnnotating();
                }
            },
            text: 'Done',
...
//        config.id = 'itemlistdetailtoolbar';
    ...
    doDoneAnnotating: function ()
    {
        ...
        if(!this.curDoneAnnotatingState)
        {
            ...
            this.active.store.getAt(this.active.index).set('doneAnnotating', true);
        }
        else
        {
            this.curDoneAnnotatingState = false;
            this.active.store.getAt(this.active.index).set('doneAnnotating', false);
        }
    ...
    setClinicalElementConfigurationId: function(clinicalElementConfigurationId){
        ...
        if(clinicalElementConfiguration)
        {
            if(!clinicalElementConfiguration.hasContent)
            {
                ...
                var btnViewInNewTab = Ext.ComponentQuery.query('[itemId="'+btnViewInNewTabId+'"]')[0];
