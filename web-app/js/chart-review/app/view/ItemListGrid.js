...
//        this.addListener('select', function(rowModel, record,  index,  eOpts){this.onSelect(rowModel, record,  index,  eOpts)});
//        this.addListener('deselect', function(rowModel, record,  index,  eOpts){this.onDeselect(rowModel, record,  index,  eOpts)});
    },
    ...
    doReconfigure: function(){
        ...
        if(clinicalElementConfiguration)
        {
            for(var key1 in this.items.items)
            {
                ...
                var plugin = comp.getPlugin('crpreview');
...
//    NOTE onSelect is being handled by Parent ItemList, not here....
