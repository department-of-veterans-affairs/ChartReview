...
Ext.define('CR.app.controller.AnnotationNatureController', {
	requires: ['Ext.util.Event', 'CR.app.controller.AnnotationNatureControllerText', 'Ext.data.reader.Xml', 'CR.app.controller.AnnotationNatureControllerAnnotations'],
...
//            Ext.getCmp('annotationlist').fireEvent('annotationschanged');
	...
	navNextPrincipal: function()
	{
		...
		if(anlt)
		{
			var comps = Ext.ComponentQuery.query('[itemId="CR.app.view.AnnotationTaskList"]');
			for(i=0; i<comps.length; i++)
			{
				var grdPnl = comps[i];
				var mdlSize = grdPnl.store.count();
				var selected = grdPnl.selModel.getSelection();
				if(selected && selected.length>0)
				{
					var selDex = grdPnl.store.indexOf(selected[0]);
					if(selDex < (mdlSize-1))
					{
						grdPnl.selModel.select(selDex+1);
					}
				}
			}
		}
	...
	classifyWholeClinicalElement: function(annotationComponent)
	{
		...
		var comps = Ext.ComponentQuery.query('[itemId="CR.app.view.AnnotationSchemaPanel"]');
...
//		CR.app.controller.AnnotationNatureControllerAnnotations.updateAnnotationWithClinicalElementDateAndName(annotation, null, annotationClinicalElementId);
	...
	configureAnnotationToolbar: function()
	{
		...
		if(this.clinicalElementId)
		{
            ...
            this.getAnnotationComponent().classificationsPanel.addListener('')
    ...
    appendAnnotationsToAnnotationToolbar: function()
	{
        if(typeof this.getAnnotationComponent().classificationsPanel != 'undefined')
        {
            ...
            var id = this.getAnnotationComponent().id + "-classificationPanel";
            var cmp = Ext.ComponentQuery.query('[id='+id+']')[0];
            var html = '';
...
//			CR.app.controller.AnnotationNatureController.ensureClinicalElementIsLoaded(this.clinicalElementId, 0, CR.app.controller.AnnotationNatureController.selectedPrincipalClinicalElement);
	...
	statics: {
		...
		{			
            ...
            // Next, get tasks from the server.

            var mdl = Ext.ModelManager.getModel('CR.app.model.AnnotationTaskModel');
            ...
            if (CR.app.controller.AnnotationNatureController.taskId) {
                ...
                CR.app.controller.AnnotationNatureController.taskId = undefined;
                CR.app.controller.AnnotationNatureController.taskType = undefined;
            } else {
			...
			else
			{
				...
				if(schemaList && schemaList.length>0 && schemaList[0].getAttributeNode('id'))
				{
					...
					else
					{
						...
						var mdl = Ext.ModelManager.getModel('CR.app.model.AnnotationSchemaModel');
		...
		setSchemaXML: function(id, xmldoc)
		{
			...
			//var comps = Ext.ComponentQuery.query('[itemId="CR.app.view.AnnotationSchemaPanel"]'); //CR.app.view.AnnotationSchemaPanel
...
//            }
		...
		blockWhileLoading: function(msg)
		{
            ...
            var apnl = Ext.getCmp('annotationcontrolpanel');
            if(apnl)
            {
...
//			var clinicalElementWidgetPanelList = Ext.ComponentQuery.query('[id="app-panel"]');
//			if(clinicalElementWidgetPanelList && clinicalElementWidgetPanelList.length>0)
//			{
//				var clinicalElementWidgetPanel = clinicalElementWidgetPanelList[0];
...
//			}
    	...
    	doViewDefSetViewDef: function(view, extraParams, me) {
    		...
    		store.getProxy().extraParams = Ext.apply(me.curViewParams, {'view': me.curViewID});
//    		store.getProxy().extraParams = Ext.apply(me.curViewParams, {'view': me.curViewID, 'timeout':2});
