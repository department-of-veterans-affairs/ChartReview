...
Role('Siesta.Test.ExtJSCore', {
    ...
    methods : {
        ...
        normalizeComponent : function(component, allowEmpty, ignoreNonVisible) {
            var Ext = this.Ext();
        ...
        normalizeElement : function(el, allowMissing, shallow, detailed) {
            ...
            if (typeof el === 'string') {
                ...
                if (el.match(/=>/)) {
                    // Composite query
                    query               = this.compositeQuery(el, null, allowMissing, true)
                    el                  = query[ 0 ]
                    matchingMultiple    = query.length > 1
                } else if (el.match(/^\s*>>/)) {
                    // Component query
                    query               = this.cq(this.trimString(el.substring(2)), allowMissing);
                    
                    var onlyVisible     = []
                    // Sencha Touch components have no "isVisible()" method
                    Joose.A.each(query, function (cmp) { 
                        if (cmp.isVisible ? cmp.isVisible() : !cmp.isHidden()) onlyVisible.push(cmp) 
                    });
                    query               = onlyVisible
        ...
        normalizeActionTarget : function (el, allowMissing) {
            ...
            if (typeof el === 'string') {
                ...
                if (mainParts.length === 2) {
                    ...
                    // Frame might not yet exist, or be ready
                    if (!scopeAdjusted || (el.match(/^\s*>>/) && !this.Ext())) {
                        this.resetScope();
                        return null;
                    }
                }
                ...
                if (el.match(/^\s*>>/)) {
                    // Component query
                    var result = this.normalizeComponent(el.substring(2), allowMissing, true);

                    if (scopeAdjusted) this.resetScope();

                    if (!result && !allowMissing) {
                        var R           = Siesta.Resource('Siesta.Test.ExtJSCore');
                        var warning     = R.get('noComponentFound') + ': ' + el;
            ...
            if (this.isExtJSComponent(el)) {
                ...
                retVal = this.SUPER(el, allowMissing);
            }
    ...
    t.clickComponentQuery('button', function () {})
        ...
        waitForTarget : function(target, callback, scope, timeout) {
            return this.SUPER(target, function() {
