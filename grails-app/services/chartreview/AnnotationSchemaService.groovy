...
class AnnotationSchemaService {
    ...
    public AnnotationSchema parseSchemaXml(String xml, boolean changeUUIDs = true) {
        ...
        def Set<AnnotationSchemaAttributeDefSortOrder> annotationSchemaAttributeDefSortOrders = this.prepareAttributeDefSortOrders(it?.attributeDefSortOrders?.annotationSchemaAttributeDefSortOrder, tAnnotationSchema, idMap);
        ...
        annotationSchemaAttributeDefSortOrders.each { it2 ->
            tAnnotationSchema.addAttributeDefSortOrder(it2);
        }
    ...
    AnnotationSchema saveSchema(String xmlToSave, boolean changeUUIDs = true) {
        ...
        annotationSchemas.each { it ->
            ...
            def tAnnotationSchema = new AnnotationSchema(id, schemaName, type, description);
            def Set<AttributeDef> attributeDefs = this.prepareAttributeDefs(it?.attributeDefs?.attributeDef, idMap);
            def Set<AnnotationSchemaAttributeDefSortOrder> annotationSchemaAttributeDefSortOrders = this.prepareAttributeDefSortOrders(it?.attributeDefSortOrders?.annotationSchemaAttributeDefSortOrder, idMap);
            def Set<ClassDef> classDefs = this.prepareClassDefs(it?.classDefs?.classDef, tAnnotationSchema, attributeDefs, idMap);
            def Set<ClassRelDef> classRelDefs = this.prepareClassRelDefs(it?.classRelDefs?.classRelDef, classDefs, attributeDefs, idMap);
    ...
    /**
     * Convert attributeDefSortOrders elements in Xml into set of domain objects
     *
     * <code>
     ...
     *        <objId>3d22f5fa-4461-4e9c-ba05-243bd1bc8f5c</objId>
     ...
     * @return set
     */
    def Set<AnnotationSchemaAttributeDefSortOrder> prepareAttributeDefSortOrders(GPathResult nodes, AnnotationSchema annotationSchema, Map idMap){
        log.info('attributeDefSortOrders')
        def set = new HashSet<AnnotationSchemaAttributeDefSortOrder>(0)
        nodes?.each { attributeDefSortOrder ->
            AnnotationSchemaAttributeDefSortOrder obj = new AnnotationSchemaAttributeDefSortOrder()
            obj.id = idMap.get(attributeDefSortOrder?.@id?.text())
            obj.sortOrder = new Integer(attributeDefSortOrder?.sortOrder?.text());
            obj.objId = attributeDefSortOrder?.objId?.text();
            obj.annotationSchema = annotationSchema;
            set.add(obj)
        }
